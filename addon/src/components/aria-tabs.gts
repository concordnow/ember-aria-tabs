...
export default class AriaTabs extends Component<AriaTabsSignature> {
  ...
  tabNodes = new TrackedArray<HTMLLIElement>();
  tabIds = new TrackedArray<string>();
  panelNodes = new TrackedArray<HTMLDivElement>();
  panelIds = new TrackedArray<string>();
  ...
  willDestroyPanel = (elementId: string, element: HTMLDivElement) => {
    this.panelNodes.filter((el) => el !== element);
    this.panelIds.filter((el) => el !== elementId);
  ...
  willDestroyTab = (elementId: string, element: HTMLLIElement) => {
    this.tabNodes.filter((el) => el !== element);
    this.tabIds.filter((el) => el !== elementId);
    this.didUpsert();
  };
...
For more information about controlled and uncontrolled mode of ember-aria-tabs see the README.`,
    ...
    if (this.mode === MODE_UNCONTROLLED) {
      ...
      if (!isNone(this.selectedIndex)) {
        newSelectedIndex = Math.min(this.selectedIndex, maxTabIndex);
      } else {
        newSelectedIndex = !isNone(this.args.defaultIndex)
          ? this.args.defaultIndex
          : 0;
      }

      this.selectedIndex = newSelectedIndex;
    } else {
      this.selectedIndex = !isNone(this.args.selectedIndex)
        ? this.args.selectedIndex
        : 0;
    }
